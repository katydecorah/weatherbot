...
test('[weatherbot] success, go outside', (assert) => {
  ...
  weatherbot.post = (message, emoji, callback) => {
    assert.equal(message, 'It\'s 71℉. Go outside!');
    assert.equal(emoji, ':cloud:');
...
test('[weatherbot] success, alert', (assert) => {
  ...
  const originalPost = weatherbot.post;
  weatherbot.post = (message, emoji, callback) => {
    assert.equal(message, '*Flash Flood Warning* from Saturday (04/29) 7:51 AM until Saturday (04/29) 12:30 PM https://alerts.weather.gov/cap/wwacapget.php?x=MO12584B4351EC.FlashFloodWarning.12584B440D08MO.SGFFFSSGF.871ed93fdad70eafa96a72b70c339adb\n*Flash Flood Watch* from Saturday (04/29) 3:29 AM until Sunday (04/30) 7:00 PM https://alerts.weather.gov/cap/wwacapget.php?x=MO12584B42AD14.FlashFloodWatch.12584F8C0B40MO.SGFFFASGF.baa9e79367797852476837c777d55f8e\n');
    assert.equal(emoji, ':bangbang:');
    assert.equal(typeof callback, 'function');
    return callback(null, 'success');
  };
...
test('[weatherbot] success, park the cars good (snow)', (assert) => {
  ...
  const originalPost = weatherbot.post;
  weatherbot.post = (message, emoji, callback) => {
    assert.equal(message, 'We\'re expected to get *4.57 inches* of snow over the next 12 hours. Park the cars good!');
    assert.equal(emoji, ':snowflake:');
    assert.equal(typeof callback, 'function');
    return callback(null, 'success');
  };
...
test('[weatherbot] do nothing', (assert) => {
  ...
  weatherbot.weather({}, null, (err, res) => {
    assert.ifError(err, 'Should not error');
    assert.deepEqual(res, 'No snow expected, it\'s not that nice out, and there are no weather alerts.', 'Message should appear in callback');
    assert.end();
  });
});
